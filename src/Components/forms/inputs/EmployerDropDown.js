    ...
    <Fragment>
      <CreatableSelect 
        ...
        onInputChange={typed=> {
          ...
          const matchedEmployers = results.filter(employer=> employer.includes(typed))
          matchedEmployers.length === 0 ? setSuccessfulySubmittedMsg('This employer will be added to our list. Make sure you typed it correctly.') : setSuccessfulySubmittedMsg('');
        }}
        value={value}
        placeholder="Type your employer name here"
      />
      {showTextOnSuccessfulSubmit.length > 0 && <p>{showTextOnSuccessfulSubmit}</p> }
    </Fragment>
  );
}
